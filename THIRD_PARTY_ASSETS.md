...
本文件记录 `dsh-kinich-theme 0.4.1` 实际使用或审阅过的素材。它是来源清单，不构成授权证明或法律意见。
...
- `Kinich_theme.png`：来自萌娘百科；未用于 `0.4.1` 的源码或发布包。
- 除 `ajaw-idle-source.png` 外的其余基尼奇/阿乔像素动画帧：来自 Bilibili UP 主“公公的日常”；未用于 `0.4.1` 的源码或发布包。
